...
void buildNewPark(Room** ret) {

  //create people
  DCooper* dc = new DCooper();
  Doug* db = new Doug();
  DXu* dx = new DXu();
  Frosh* frosh1 = new Frosh();
  Frosh* frosh2 = new Frosh();
  Haverbro* haverbro1 = new Haverbro();
  HMajor* hmajor1 = new HMajor();
  ...
  Room* dxulab = new Room("Diana's Lab",NULL,NULL,NULL,hallway2,NULL,NULL);
  Item* chocolate = new Item("Chocolate","A fancy chocolate truffle",false,5);
  ...
  //...moved into the array passed in (Sam do we need these things? I don't think they are being used...)
  for(int i=0; i< 32; ++i){
    ret[i] = rettby[i];
  }
